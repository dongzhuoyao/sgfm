...
class UViT(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # print("timestep range.,", timesteps.min(), timesteps.max())
        if "edit_loc" in kwargs and kwargs["edit_loc"] == "head":
            x = dissect_helper_uvit(x=x, timesteps=timesteps, block_id=0, **kwargs)

        # if "collect_feat" in kwargs:
        feat_list = []
        ...
        if "edit_loc" in kwargs and kwargs["edit_loc"] == "mid":
            x = dissect_helper_uvit(x=x, timesteps=timesteps, block_id=0, **kwargs)
        ...
        if "edit_loc" in kwargs and kwargs["edit_loc"] == "tail":
            x = dissect_helper_uvit(x=x, timesteps=timesteps, block_id=0, **kwargs)
        return x, None if layerid is None else feat_list[layerid]
